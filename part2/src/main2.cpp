...
public:
    serial(boost::asio::io_service& io_service, unsigned int baud, const string& device, Arduino& arduinoIn)
        :active_(true),
         io(io_service),
         serialPort(io_service),
         arduino(arduinoIN)
    {
       boost::system::error_code ec;serialPort.open(port,ec);		 //connect to port
       if (!ec)
          {
             cerr << "Failed to open serial port\n";
          }
       serialPort.set_option(serial_port_base::baud_rate(baud),ec);
       read_start();
